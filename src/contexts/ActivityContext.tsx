...
export const ActivityProvider = ({ children }: { children: React.ReactNode }) => {
  ...
  useEffect(() => {
    if (user) {
      ...
      if (stored) {
        ...
        localStorage.setItem(storageKey, JSON.stringify({ activities: mockActivities }));
      }
    }
  }, [user]);
  ...
  const addActivity = () => {
    ...
    const storageKey = `zenith_activities_${user.id}`;
    localStorage.setItem(storageKey, JSON.stringify({ activities: updatedActivities }));
  };

  return (
    <ActivityContext.Provider value={{ activities, currentStreak, longestStreak, addActivity }}>
      {children}
